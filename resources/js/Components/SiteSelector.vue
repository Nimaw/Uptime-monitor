<template>
            ...
            <ul class="-space-y-1">
                <li v-for="site in sites" :key="site.id">
                    <Link class="px-4 py-2 hover:bg-gray-100 block text-sm  text-gray-500 hover:text-gray-700"
                        :href="`/dashboard/${site.id}`">{{ site.domain }}</Link>
                </li>
                <li>
                    <button v-on:click="showNewSiteModal = true; hide()"
                        class="block w-full text-left px-4 py-2 hover:bg-gray-100 text-indigo-500 front-bold text-sm">
                        Add a site
                    </button>
                </li>
            </ul>
    ...
    <VueFinalModal v-model="showNewSiteModal" classes="flex justify-center items-start pt-16 md:pt-24 mx-4"
        ...
        <form v-on:submit.prevent="createSite" class="overflow-hidden space-y-4">
            ...
            <TextInput id="domain" type="text" class="block w-full h-9 text-sm" placeholder="e.g https://nima.js.org"
                v-model="siteForm.domain" :class="{ 'border-red-500' : siteForm.errors.domain}" />
